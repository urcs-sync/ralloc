...
void TCacheBin::push_block(char* block)
{
	...
	*(pptr<char>*)block = _block==0 ? nullptr : (char*)(_block + (uint64_t)rpmalloc::mgr->base_addr);
	_block = (uint64_t)block - (uint64_t)rpmalloc::mgr->base_addr;
	_block_num++;
}
...
char* TCacheBin::pop_block()
{
	...
	char* ret = (char*)(_block + (uint64_t)rpmalloc::mgr->base_addr);
	char* next = (char*)(*(pptr<char>*)(_block + (uint64_t)rpmalloc::mgr->base_addr));
	_block = next == nullptr ? 0 : (uint64_t) next - (uint64_t)rpmalloc::mgr->base_addr;
	_block_num--;
	return ret;
}
...
void TCacheBin::pop_list(char* block, uint32_t length)
{
	...
	_block = block==nullptr? 0: (uint64_t)block - (uint64_t)rpmalloc::mgr->base_addr;
	_block_num -= length;
}
