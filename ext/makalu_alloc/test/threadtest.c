...
int main(){
    ...
    MAK_pthread_create(&thread_id,&attr,&go_alloc,(void*)&arg);
    MAK_pthread_join(thread_id,NULL);
    MAK_pthread_create(&thread_id,&attr,&go_alloc,(void*)&arg);
    MAK_pthread_join(thread_id,NULL);
    // std::thread first(go_alloc,1000);
    // std::thread second(go_alloc,1000);
    // std::thread third(go_alloc,1000);
    // first.join();
    // second.join();
    // third.join();
    go_alloc((void*)&arg);
